function [kr, mu, D] = ComputeCoefficientsParent(E,Te,T,N,r_interfaces,r_nodes,MuConst)
...
mu = zeros(size(Ec,1)+1, 6);
D = zeros(size(Ec,1)+1, 6);
...
if isequal(MuConst, -1)
    ...
    mu(:,6) = (1/N) * min(0.97e23 * (T).^(-0.5), 3.56e19 * (abs(E)/N).^(-0.1));
